...
public class CPL {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("X")) {
                    ...
                    if(line.isEmpty()) {
                        ...
                        cpl = getCharacteristicPathLength(graph);
                        result.add(cpl);
    ...
    private static double getCharacteristicPathLength(Graph<String, DefaultEdge> graph) {
        FloydWarshallShortestPaths<String, DefaultEdge> shortestPathAlgo = new FloydWarshallShortestPaths<>(graph);

        double cplAverageLength = 0.0;
            ...
            for (String shortestPathVertex : graph.vertexSet()) {
                if (!shortestPathVertex.equals(vertex)) {
                    ...
                    if (pathPerVertex != null) {
                        averagePathLength += pathPerVertex.getLength();
                    }
                }
            }
            if (shortestPathAlgo.getShortestPathsCount() != 0) {
                averagePathLength = averagePathLength / shortestPathAlgo.getShortestPathsCount();
            }
            cplAverageLength += averagePathLength;

        }
        return cplAverageLength;
    }
