...
public class DegreeDis {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (line.startsWith("X")) {
                    ...
                    if(line.isEmpty()) {
                        ...
                        List<Double> dd = getNodeDegrees(graph);

                        // Add the node degrees to the accumulating result list
                        result.addAll(dd);
    ...
    private static List<Double> getNodeDegrees(Graph<String, DefaultEdge> graph) {
        List<Double> nodeDegrees = new ArrayList<>();
    
        for (String vertex : graph.vertexSet()) {
            // Convert the degree to Double and add it to the list
            nodeDegrees.add((double) graph.edgesOf(vertex).size());
        }
